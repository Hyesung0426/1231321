...
function renderRestaurantInfo(restaurants) {
    ...
    restaurants.forEach((placeDetails) => {
        ...
        //     saveRestaurantToList(placeDetails);
        // });
...
function saveRestaurantToList(placeDetails) {
    const restaurant = {
    ...
    fetch('/api/saveRestaurantToList', {
        method: 'POST',
        headers: {
        ...
        .then(response => response.text()) // 변경된 부분
        .then(text => {
            try {
                const data = JSON.parse(text); // JSON 파싱 시도
                alert('리스트에 저장되었습니다!');
            } catch (error) {
                console.error('Invalid JSON:', text); // 유효하지 않은 JSON일 경우 오류 출력
            }
        })
        .catch(error => {
